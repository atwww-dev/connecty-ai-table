  ...
  class ComparisonTable {
    ...
    createFeatureRow(feature, categoryIndex, featureIndex) {
      ...
      headerKeys.forEach((key, i) => {
        ...
        // Create consistent icon container structure
        ...
        if (typeof value === "object") {
          iconWrapper.innerHTML = this.getStatusIcon(value.type);
          iconContainer.appendChild(iconWrapper);

          if (value.text && value.text.trim() !== "") {
            const textSpan = document.createElement("span");
            textSpan.className = "feature-check-text";
            textSpan.textContent = value.text;
            iconContainer.appendChild(textSpan);
          }
        } else {
          iconWrapper.innerHTML = this.getStatusIcon(value);
          iconContainer.appendChild(iconWrapper);
        }
